...
namespace Client
{
	...
	class ClientMessenger : AsynchronousMessenger<ServerToClientMessage, ClientToServerMessage>
	{
		...
		public ClientMessenger(Socket socket, PlayerPreferences preferences) :
			base(socket)
		{
		...
		void OnGameException(GameException exception)
		{
			throw new NotImplementedException();
		}
		...
		void OnGameStarted(ServerToClientMessage message)
		{
			...
			_Game = new Game(start.Black.Deployment, start.White.Deployment);
		...
		void OnPieceMoved(ServerToClientMessage message)
		{
			...
			_Game.MovePiece(move.Source, move.Destination);
		}

		void OnPieceDropped(ServerToClientMessage message)
		{
			PieceDrop drop = message.Drop;
			_Game.DropPiece(drop.Destination, drop.Piece);
		}
